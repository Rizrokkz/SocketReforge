    ...
    protected void tick0(boolean firstRun, float time, @NonNullDecl InteractionType type,
        ...
        if (type == InteractionType.Secondary) {
            if (ResonantCompendiumUI.isAvailable()) {
                ...
                player.getPlayerRef().sendMessage(Message.raw("HyUI not installed - compendium UI disabled."));
            }
            return;
        }
        ...
        if (absorbed > 0) {
            ...
            player.getPlayerRef().sendMessage(Message.raw(countSummary.replace("\n", " ")));
            ...
            player.getPlayerRef().sendMessage(Message.raw("Compendium absorbed " + absorbed + " shard" + (absorbed == 1 ? "" : "s") + "!"));
            ...
            player.getPlayerRef().sendMessage(Message.raw("Compendium contents: " + complete + " complete, " + incomplete + " incomplete recipes."));
            if (data.isEmpty()) {
                player.getPlayerRef().sendMessage(Message.raw("The compendium is currently empty. Right-click to absorb recipe shards in your inventory."));
            }
        }
    }
